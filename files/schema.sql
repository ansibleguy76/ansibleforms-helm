-- AnsibleForms base schema.
--
-- Applied automatically to the MySQL the chart deploys. If you run your own
-- database (mysql.enabled: false), apply this once against it before starting
-- AnsibleForms: the application migrates an existing schema forward but does
-- not create it from nothing, and without these tables it starts, answers on
-- its port and then fails every query.
--
-- Safe to re-run: nothing is dropped and every table is created only if it is
-- missing. It does not grant anything, that is up to whoever owns the server.
...
USE `AnsibleForms`;
CREATE TABLE IF NOT EXISTS `awx` (
...
USE `AnsibleForms`;
CREATE TABLE IF NOT EXISTS `azuread` (
